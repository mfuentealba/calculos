...
binance.websockets.depthCache(["NEOETH"], function(symbol, depth) {
	let max = 10; // Show 10 closest orders only
	let bids = binance.sortBids(depth.bids, max);
	let asks = binance.sortAsks(depth.asks, max);
	/*console.log(symbol+" depth cache update");
	...
	console.log("ask: "+binance.first(asks));
	console.log("bid: "+binance.first(bids));*/
	fnCruce('bin', asks, 'NEOETH', 'buy');
	fnCruce('bin', bids, 'NEOETH_', 'sell');
});
	...
	/*if(swOrd == true){
		...
		binance.cancel(order.symbol, order.orderId, (error, response, symbol) => {
		  //console.log(symbol+" cancel response:", response);
			fsLauncher.appendFileSync('./NEOETH.txt', "CANCELACION DE ORDEN " + order.orderId + '\n' + msg + '\n' + JSON.stringify(response) + " \n", (err) => {
						if (err) throw err;
							////console.log('The "data to append" was appended to file!');
						}); 
			if(error && error.body){
				console.log(error.body);
				fsLauncher.appendFileSync('./NEOETH.txt', JSON.stringify(error.body) + " \n", (err) => {
						if (err) throw err;
							////console.log('The "data to append" was appended to file!');
						});  
				swOrd = true;
...
var str;
function fnNormal(){
	if(validacionDatos['ETHUSDT'] && validacionDatos['NEOETH'] && validacionDatos['NEOUSDT']
	&& validacionDatos['ETHUSDT_'] && validacionDatos['NEOUSDT_'] && validacionDatos['NEOETH_']
	){
		//console.log(order);
		...
		if(order != null){
			...
			*/
			if(order.orderId != null){
				...
				if(order.symbol == 'NEOETH'){
					...
					*/
					...
					var qty3 = qty2 * 0.999;// * binance.first(validacionDatos['NEOETH_'].data);
					str = String(qty3).split(".");
					qty3 = Number(str[0] + '.' + str[1].substr(0, objDecimales['NEOUSDT']))
					...
					var volAnt = 0;
					for(let obj in validacionDatos['NEOETH_'].data){
						if(order.price == validacionDatos['NEOETH_'].data[obj]){
							break;
						} else {
							volAnt += Number(validacionDatos['NEOETH_'].data[obj]);	
						}
					...
					if(result < 0){
						...
						//fnCancel(qty1, px1, qty3, px3, 'ETHUSDT', 'NEOUSDT', " Se Pierde dif: " + result);
						fnConsulta(qty1, px1, qty3, px3, 'ETHUSDT', 'NEOUSDT', " Se Pierde dif: " + result, result, 'buy');
						
					} else if(volAnt > order.origQty * 500 && order.status == 'NEW'){
						//fnCancel(qty1, px1, qty3, px3, 'ETHUSDT', 'NEOUSDT', " Se Pierde Posicion");
						fnConsulta(qty1, px1, qty3, px3, 'ETHUSDT', 'NEOUSDT', " Se Pierde Posicion", -10, 'buy');
					} else {
						fnConsulta(qty1, px1, qty3, px3, 'ETHUSDT', 'NEOUSDT', " COnsulta ordinaria ", 10, 'buy');
					}
					...
					*/
					...
					var qty6 = qty5 * 0.999;// * binance.first(validacionDatos['NEOETH_'].data);
					str = String(qty6).split(".");
					qty6 = Number(str[0] + '.' + str[1].substr(0, objDecimales['NEOETH']))
					
					
					var px6 = binance.first(validacionDatos['NEOETH_'].data);
					...
					if(result2 < 0){
						...
						//fnCancel(qty4, px4, qty6, px6, 'ETHUSDT', 'NEOETH', " Se Pierde dif: " + result2);
						fnConsulta(qty4, px4, qty6, px6, 'ETHUSDT', 'NEOETH', " Se Pierde dif: " + result2, result2, 'sell');
						
					} else if(volAnt > order.origQty * 500 && order.status == 'NEW'){
						//fnCancel(qty4, px4, qty6, px6, 'ETHUSDT', 'NEOETH', " Se Pierde Posicion");
						fnConsulta(qty4, px4, qty6, px6, 'ETHUSDT', 'NEOETH', " Se Pierde Posicion", -10, 'sell');
					} else {
						fnConsulta(qty4, px4, qty6, px6, 'ETHUSDT', 'NEOETH', " Consulta ordinaria", 10, 'sell');
					}
			...
			var px5 = binance.first(validacionDatos['NEOUSDT'].data);//lowestAsk;
			...
			if(result > 0.000001 && result > result2){
				...
				binance.buy("NEOETH", qty2, px2, {type:'LIMIT'}, (error, response) => {
				
					countOrd++;
					console.log("Limit Buy response", response);
					fsLauncher.appendFileSync('./NEOETH.txt',"Limit Buy response, " + JSON.stringify(response) + " \n", (err) => {
							if (err) throw err;
								////console.log('The "data to append" was appended to file!');
							});
					...
					if(error && error.body){
						console.log(error.body);
						order = null;
						fsLauncher.appendFileSync('./NEOETH.txt', 'ETHUSDT\n' + JSON.stringify(error.body) + " \n", (err) => {
							if (err) throw err;
								////console.log('The "data to append" was appended to file!');
							});  
					} else {
						...
						if(order.status == 'FILLED'){
							fnRemate(qty1, px1, qty3, px3, "ETHUSDT", "NEOUSDT", 'xxxx', 'buy');
							order = null;
						} else {
							ord1 = {px: px1, qty: qty1, symbol: 'ETHUSDT'};
							ord2 = {px: px3, qty: qty3, symbol: 'NEOUSDT'};
							fnConsulta(qty1, px1, qty3, px3, "ETHUSDT", "NEOUSDT", 'ConsultaOrdinaria', result, 'buy');
						}
					}
			...
			if(result2 > 0.000001){
				...
				binance.buy("NEOUSDT", qty5, px5, {type:'LIMIT'}, (error, response) => {
				  ...
				  if(error && error.body){
					console.log(error.body);
					fsLauncher.appendFileSync('./NEOETH.txt', 'NEOUSDT\n' + JSON.stringify(error.body) + " \n", (err) => {
							if (err) throw err;
								////console.log('The "data to append" was appended to file!');
							});  
				  } else {
						order = response;
						if(order.status == 'FILLED'){
							fnRemate(qty4, px4, qty6, px6, "ETHUSDT", "NEOETH", 'xxxx', 'sell');
							order = null;
						} else {
							ord1 = {px: px4, qty: qty4, symbol: 'ETHUSDT'};
							ord2 = {px: px6, qty: qty6, symbol: 'NEOETH'};
							fnConsulta(qty4, px4, qty6, px6, "ETHUSDT", "NEOETH", 'Consulta Ordinaria INI', result2, 'sell');
						}
				  }	
			...
			if(((result > 0 ) || (result2 > 0 )) && !sw){
				...
				fsLauncher.appendFileSync('./NEOETH.txt', " [ " + (acum) + " :: " + (acum2) + " ]\n", (err) => {
							if (err) throw err;
								////console.log('The "data to append" was appended to file!');
							});
			}
...
function fnRemateFallo(){
	...
	//console.log('qty2: ' + ord1.qty);
		fsLauncher.appendFileSync('./NEOETH.txt',"ORDEN VENTA, " + ord1.symbol + ", " + ord1.qty + ", " + ord1.px + " \n", (err) => {
						if (err) throw err;
							////console.log('The "data to append" was appended to file!');
						});
	...
	binance[opG](ord1.symbol, ord1.qty, ord1.px, {type:'LIMIT'}, (error, response) => {
	  ...
	  if(error && error.body){
		console.log(error.body);
		fsLauncher.appendFileSync('./NEOETH.txt', ord1.symbol + '\n' + JSON.stringify(error.body) + " \n", (err) => {
				if (err) throw err;
					////console.log('The "data to append" was appended to file!');
				});  
	  } else {
	...
	fsLauncher.appendFileSync('./NEOETH.txt',"ORDEN VENTA, " + ord2.symbol + ", " + ord2.qty + ", " + ord2.px + " \n", (err) => {
						if (err) throw err;
							////console.log('The "data to append" was appended to file!');
						});
	binance.sell(ord2.symbol, ord2.qty, ord2.px, {type:'LIMIT'}, (error, response) => {
		...
		if(error && error.body){
			console.log(error.body);
			fsLauncher.appendFileSync('./NEOETH.txt', ord2.symbol + '\n' + JSON.stringify(error.body) + " \n", (err) => {
				if (err) throw err;
					////console.log('The "data to append" was appended to file!');
				});  
		} else {
...
function fnEspera(){
	...
	for(let str in objOrdenRemateFallo){
		...
		binance.orderStatus(ordA.symbol, ordA.orderId, (error, orderStatus, symbol) => {
			...
			fsLauncher.appendFileSync('./NEOETH.txt', symbol + " order status: " + JSON.stringify(orderStatus) + " \n", (err) => {
					if (err) throw err;
						////console.log('The "data to append" was appended to file!');
					});  
			console.log("******** consultando orden " + order.orderId + " ******* --> " + swOrd);
...
function fnRemate(qty2, px2, qty3, px3, symbol1, symbol2, msg, op){
	...
	//console.log('qty2: ' + qty2);
	fsLauncher.appendFileSync('./NEOETH.txt',"ORDEN VENTA, " + symbol1 + ", " + qty2 + ", " + px2 + " \n", (err) => {
						if (err) throw err;
							////console.log('The "data to append" was appended to file!');
						});
	...
	binance[op](symbol1, qty2, px2, {type:'LIMIT'}, (error, response) => {
	  ...
	  if(error && error.body){
		console.log(error.body);
		fsLauncher.appendFileSync('./NEOETH.txt', 'NEOETH\n' + JSON.stringify(error.body) + " \n", (err) => {
				if (err) throw err;
					////console.log('The "data to append" was appended to file!');
				});  
	  }
	...
	fsLauncher.appendFileSync('./NEOETH.txt',"ORDEN VENTA, " + symbol2 + ", " + qty3 + ", " + px3 + " \n", (err) => {
						if (err) throw err;
							////console.log('The "data to append" was appended to file!');
						});
...
function fnConsulta(qty2, px2, qty3, px3, symbol1, symbol2, msg, result, op){
	...
	if(swOrd == true){
		...
		binance.orderStatus(order.symbol, order.orderId, (error, orderStatus, symbol) => {
			console.log(symbol + " order status:", orderStatus);
			fsLauncher.appendFileSync('./NEOETH.txt', symbol + " DIF: " +  result + " order status: " + JSON.stringify(orderStatus) + " \n", (err) => {
					if (err) throw err;
						////console.log('The "data to append" was appended to file!');
					});  
			console.log("******** consultando orden " + order.orderId + " ******* --> " + swOrd);
			...
			if(orderStatus.status == 'FILLED'){
				...
				if(result == -10){
					fsLauncher.appendFileSync('./NEOETH.txt', "ORDEN TAPADA \n", (err) => {
						if (err) throw err;
							////console.log('The "data to append" was appended to file!');
						});
				} else {
