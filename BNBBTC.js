	...
	/*if(swOrd == true){
		...
		binance.cancel(order.symbol, order.orderId, (error, response, symbol) => {
		  //console.log(symbol+" cancel response:", response);
			fsLauncher.appendFileSync('./BNBBTC.txt', "CANCELACION DE ORDEN " + order.orderId + '\n' + msg + '\n' + JSON.stringify(response) + " \n", (err) => {
						if (err) throw err;
							////console.log('The "data to append" was appended to file!');
						}); 
			if(error && error.body){
				console.log(error.body);
				fsLauncher.appendFileSync('./BNBBTC.txt', JSON.stringify(error.body) + " \n", (err) => {
						if (err) throw err;
							////console.log('The "data to append" was appended to file!');
						});  
				swOrd = true;
			...
			*/
			if(order.orderId != null){
				...
				if(order.symbol == 'BNBBTC'){
					...
					qty2 = 1;//Number(str[0] + '.' + str[1].substr(0, objDecimales['BNBBTC']))
					*/
			...
			swOrd = true;
			//console.log("ORDER: " + countOrd);
			var qty2 = 1;//qty1 * 0.999 / binance.first(validacionDatos['BNBBTC_'].data);//lowestAsk;
			/*str = String(qty2).split(".");
			...
			var qty1 = ((qty2 * px2) / 0.999) + Math.pow(10, -objDecimales['BTCUSDT']);//lowestAsk; //11.9 / (Number(binance.first(validacionDatos['BTCUSDT'].data)) + 0.000001);
			str = String(qty1).split(".");
			qty1 = Number(str[0] + '.' + str[1].substr(0, objDecimales['BTCUSDT']));
			//qty1 = qty1.toFixed(objDecimales['BTCUSDT']);
			//console.log(qty1);
			...
			var qty5 = 1;//amount4 / binance.first(validacionDatos['BNBUSDT'].data);//lowestAsk;
			...
			if(result > 0.000001 && result > result2){
				...
				binance.buy("BNBBTC", qty2, px2, {type:'LIMIT'}, (error, response) => {
					...
					fsLauncher.appendFileSync('./BNBBTC.txt',"Limit Buy response, " + JSON.stringify(response) + " \n", (err) => {
							if (err) throw err;
								////console.log('The "data to append" was appended to file!');
							});
					...
					if(error && error.body){
						console.log(error.body);
						order = null;
						fsLauncher.appendFileSync('./BNBBTC.txt', 'BTCUSDT\n' + JSON.stringify(error.body) + " \n", (err) => {
							if (err) throw err;
								////console.log('The "data to append" was appended to file!');
							});  
					} else {
			...
			if(result2 > 0.000001){
				...
				binance.buy("BNBUSDT", qty5, px5, {type:'LIMIT'}, (error, response) => {
				  ...
				  if(error && error.body){
					console.log(error.body);
					fsLauncher.appendFileSync('./BNBBTC.txt', 'BNBUSDT\n' + JSON.stringify(error.body) + " \n", (err) => {
							if (err) throw err;
								////console.log('The "data to append" was appended to file!');
							});  
				  } else {
			...
			if(((result > 0 ) || (result2 > 0 )) && !sw){
				...
				fsLauncher.appendFileSync('./BNBBTC.txt', " [ " + (acum) + " :: " + (acum2) + " ]\n", (err) => {
							if (err) throw err;
								////console.log('The "data to append" was appended to file!');
							});
			}
...
function fnRemateFallo(){
	...
	//console.log('qty2: ' + ord1.qty);
		fsLauncher.appendFileSync('./BNBBTC.txt',"ORDEN VENTA, " + ord1.symbol + ", " + ord1.qty + ", " + ord1.px + " \n", (err) => {
						if (err) throw err;
							////console.log('The "data to append" was appended to file!');
						});
	...
	binance[opG](ord1.symbol, ord1.qty, ord1.px, {type:'LIMIT'}, (error, response) => {
	  ...
	  if(error && error.body){
		console.log(error.body);
		fsLauncher.appendFileSync('./BNBBTC.txt', ord1.symbol + '\n' + JSON.stringify(error.body) + " \n", (err) => {
				if (err) throw err;
					////console.log('The "data to append" was appended to file!');
				});  
	  } else {
	...
	fsLauncher.appendFileSync('./BNBBTC.txt',"ORDEN VENTA, " + ord2.symbol + ", " + ord2.qty + ", " + ord2.px + " \n", (err) => {
						if (err) throw err;
							////console.log('The "data to append" was appended to file!');
						});
	binance.sell(ord2.symbol, ord2.qty, ord2.px, {type:'LIMIT'}, (error, response) => {
		...
		if(error && error.body){
			console.log(error.body);
			fsLauncher.appendFileSync('./BNBBTC.txt', ord2.symbol + '\n' + JSON.stringify(error.body) + " \n", (err) => {
				if (err) throw err;
					////console.log('The "data to append" was appended to file!');
				});  
		} else {
...
function fnEspera(){
	...
	for(let str in objOrdenRemateFallo){
		...
		binance.orderStatus(ordA.symbol, ordA.orderId, (error, orderStatus, symbol) => {
			...
			fsLauncher.appendFileSync('./BNBBTC.txt', symbol + " order status: " + JSON.stringify(orderStatus) + " \n", (err) => {
					if (err) throw err;
						////console.log('The "data to append" was appended to file!');
					});  
...
function fnRemate(qty2, px2, qty3, px3, symbol1, symbol2, msg, op){
	...
	//console.log('qty2: ' + qty2);
	fsLauncher.appendFileSync('./BNBBTC.txt',"ORDEN VENTA, " + symbol1 + ", " + qty2 + ", " + px2 + " \n", (err) => {
						if (err) throw err;
							////console.log('The "data to append" was appended to file!');
						});
	...
	binance[op](symbol1, qty2, px2, {type:'LIMIT'}, (error, response) => {
	  ...
	  if(error && error.body){
		console.log(error.body);
		fsLauncher.appendFileSync('./BNBBTC.txt', 'BNBBTC\n' + JSON.stringify(error.body) + " \n", (err) => {
				if (err) throw err;
					////console.log('The "data to append" was appended to file!');
				});  
	  }
	...
	fsLauncher.appendFileSync('./BNBBTC.txt',"ORDEN VENTA, " + symbol2 + ", " + qty3 + ", " + px3 + " \n", (err) => {
						if (err) throw err;
							////console.log('The "data to append" was appended to file!');
						});
...
function fnConsulta(qty2, px2, qty3, px3, symbol1, symbol2, msg, result, op){
	...
	if(swOrd == true){
		...
		binance.orderStatus(order.symbol, order.orderId, (error, orderStatus, symbol) => {
			console.log(symbol + " order status:", orderStatus);
			fsLauncher.appendFileSync('./BNBBTC.txt', symbol + " DIF: " +  result + " order status: " + JSON.stringify(orderStatus) + " \n", (err) => {
					if (err) throw err;
						////console.log('The "data to append" was appended to file!');
					});  
			console.log("******** consultando orden " + order.orderId + " ******* --> " + swOrd);
			...
			if(orderStatus.status == 'FILLED'){
				...
				if(result == -10){
					fsLauncher.appendFileSync('./BNBBTC.txt', "ORDEN TAPADA \n", (err) => {
						if (err) throw err;
							////console.log('The "data to append" was appended to file!');
						});
				} else {
