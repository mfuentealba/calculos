...
binance.websockets.depthCache(["QTUMBTC"], function(symbol, depth) {
	let max = 1; // Show 10 closest orders only
	let bids = binance.sortBids(depth.bids, max);
	let asks = binance.sortAsks(depth.asks, max);
	/*console.log(symbol+" depth cache update");
	...
	console.log("ask: "+binance.first(asks));
	console.log("bid: "+binance.first(bids));*/
	fnCruce('bin', asks, 'QTUMBTC', 'buy');
	fnCruce('bin', bids, 'QTUMBTC_', 'sell');
});

binance.websockets.depthCache(["QTUMUSDT"], function(symbol, depth) {
	let max = 1; // Show 10 closest orders only
	let bids = binance.sortBids(depth.bids, max);
	let asks = binance.sortAsks(depth.asks, max);
	/*console.log(symbol+" depth cache update");
	...
	console.log("ask: "+binance.first(asks));
	console.log("bid: "+binance.first(bids));*/
	fnCruce('bin', bids, 'QTUMUSDT', 'sell');
	fnCruce('bin', asks, 'QTUMUSDT_', 'buy');
});
	...
	/*if(swOrd == true){
		...
		binance.cancel(order.symbol, order.orderId, (error, response, symbol) => {
		  //console.log(symbol+" cancel response:", response);
			fsLauncher.appendFileSync('./QTUMBTC.txt', "CANCELACION DE ORDEN " + order.orderId + '\n' + msg + '\n' + JSON.stringify(response) + " \n", (err) => {
						if (err) throw err;
							////console.log('The "data to append" was appended to file!');
						}); 
			if(error && error.body){
				console.log(error.body);
				fsLauncher.appendFileSync('./QTUMBTC.txt', JSON.stringify(error.body) + " \n", (err) => {
						if (err) throw err;
							////console.log('The "data to append" was appended to file!');
						});  
				swOrd = true;
...
var str;
function fnNormal(){
	if(validacionDatos['BTCUSDT'] && validacionDatos['QTUMBTC'] && validacionDatos['QTUMUSDT']
	&& validacionDatos['BTCUSDT_'] && validacionDatos['QTUMUSDT_'] && validacionDatos['QTUMBTC_']
	){
		//console.log(order);
		...
		if(order != null){
			...
			*/
			if(order.orderId != null){
				...
				if(order.symbol == 'QTUMBTC'){
					console.log("ORDER: " + countOrd);
					
					var qty2 = 1;//qty1 * 0.999 / order.price;//lowestAsk;
					/*str = String(qty2).split(".");
					qty2 = 1;//Number(str[0] + '.' + str[1].substr(0, objDecimales['QTUMBTC']))
					*/
					...
					var qty3 = qty2 * 0.999;// * binance.first(validacionDatos['QTUMBTC_'].data);
					str = String(qty3).split(".");
					qty3 = Number(str[0] + '.' + str[1].substr(0, objDecimales['QTUMUSDT']))
					
					var px3 = binance.first(validacionDatos['QTUMUSDT'].data);
					...
					var volAnt = 0;
					for(let obj in validacionDatos['QTUMBTC_'].data){
						if(order.price == validacionDatos['QTUMBTC_'].data[obj]){
							break;
						} else {
							volAnt += Number(validacionDatos['QTUMBTC_'].data[obj]);	
						}
					...
					if(result < 0){
						...
						//fnCancel(qty1, px1, qty3, px3, 'BTCUSDT', 'QTUMUSDT', " Se Pierde dif: " + result);
						fnConsulta(qty1, px1, qty3, px3, 'BTCUSDT', 'QTUMUSDT', " Se Pierde dif: " + result, result, 'buy');
						
					} else if(volAnt > order.origQty * 500 && order.status == 'NEW'){
						//fnCancel(qty1, px1, qty3, px3, 'BTCUSDT', 'QTUMUSDT', " Se Pierde Posicion");
						fnConsulta(qty1, px1, qty3, px3, 'BTCUSDT', 'QTUMUSDT', " Se Pierde Posicion", -10, 'buy');
					} else {
						fnConsulta(qty1, px1, qty3, px3, 'BTCUSDT', 'QTUMUSDT', " COnsulta ordinaria ", 10, 'buy');
					}
					
					
				} else if(order.symbol == 'QTUMUSDT'){
					
					var qty5 = 1;//amount4 / order.price;//lowestAsk;
					/*str = String(qty5).split(".");
					qty5 = Number(str[0] + '.' + str[1].substr(0, objDecimales['QTUMUSDT']))
					*/
					...
					var qty6 = qty5 * 0.999;// * binance.first(validacionDatos['QTUMBTC_'].data);
					str = String(qty6).split(".");
					qty6 = Number(str[0] + '.' + str[1].substr(0, objDecimales['QTUMBTC']))
					
					
					var px6 = binance.first(validacionDatos['QTUMBTC_'].data);
					...
					if(result2 < 0){
						...
						//fnCancel(qty4, px4, qty6, px6, 'BTCUSDT', 'QTUMBTC', " Se Pierde dif: " + result2);
						fnConsulta(qty4, px4, qty6, px6, 'BTCUSDT', 'QTUMBTC', " Se Pierde dif: " + result2, result2, 'sell');
						
					} else if(volAnt > order.origQty * 500 && order.status == 'NEW'){
						//fnCancel(qty4, px4, qty6, px6, 'BTCUSDT', 'QTUMBTC', " Se Pierde Posicion");
						fnConsulta(qty4, px4, qty6, px6, 'BTCUSDT', 'QTUMBTC', " Se Pierde Posicion", -10, 'sell');
					} else {
						fnConsulta(qty4, px4, qty6, px6, 'BTCUSDT', 'QTUMBTC', " Consulta ordinaria", 10, 'sell');
					}
			...
			swOrd = true;
			//console.log("ORDER: " + countOrd);
			var qty2 = 1;//qty1 * 0.999 / binance.first(validacionDatos['QTUMBTC_'].data);//lowestAsk;
			/*str = String(qty2).split(".");
			qty2 = Number(str[0] + '.' + str[1].substr(0, objDecimales['QTUMBTC']))*/
			var px2 = binance.first(validacionDatos['QTUMBTC_'].data);//lowestAsk;
			var amount2 = qty2 * px2 * 0.999;
			amount2 = amount2.toFixed(8);
			//console.log("px: " + px2 + ", qty: " + qty2 + ", amount: " + amount2);
			...
			var qty5 = 1;//amount4 / binance.first(validacionDatos['QTUMUSDT'].data);//lowestAsk;
			
			/*str = String(qty5).split(".");
			qty5 = Number(str[0] + '.' + str[1].substr(0, objDecimales['QTUMUSDT']))*/
					
			var px5 = binance.first(validacionDatos['QTUMUSDT'].data);//lowestAsk;
			...
			if(result > 0.000001 && result > result2){
				...
				binance.buy("QTUMBTC", qty2, px2, {type:'LIMIT'}, (error, response) => {
				
					countOrd++;
					console.log("Limit Buy response", response);
					fsLauncher.appendFileSync('./QTUMBTC.txt',"Limit Buy response, " + JSON.stringify(response) + " \n", (err) => {
							if (err) throw err;
								////console.log('The "data to append" was appended to file!');
							});
					...
					if(error && error.body){
						console.log(error.body);
						order = null;
						fsLauncher.appendFileSync('./QTUMBTC.txt', 'BTCUSDT\n' + JSON.stringify(error.body) + " \n", (err) => {
							if (err) throw err;
								////console.log('The "data to append" was appended to file!');
							});  
					} else {
						...
						if(order.status == 'FILLED'){
							fnRemate(qty1, px1, qty3, px3, "BTCUSDT", "QTUMUSDT", 'xxxx', 'buy');
							order = null;
						} else {
							ord1 = {px: px1, qty: qty1, symbol: 'BTCUSDT'};
							ord2 = {px: px3, qty: qty3, symbol: 'QTUMUSDT'};
							fnConsulta(qty1, px1, qty3, px3, "BTCUSDT", "QTUMUSDT", 'ConsultaOrdinaria', result, 'buy');
						}
					}
			...
			if(result2 > 0.000001){
				order = {};
				console.log('\u0007');
				binance.buy("QTUMUSDT", qty5, px5, {type:'LIMIT'}, (error, response) => {
				  ...
				  if(error && error.body){
					console.log(error.body);
					fsLauncher.appendFileSync('./QTUMBTC.txt', 'QTUMUSDT\n' + JSON.stringify(error.body) + " \n", (err) => {
							if (err) throw err;
								////console.log('The "data to append" was appended to file!');
							});  
				  } else {
						order = response;
						if(order.status == 'FILLED'){
							fnRemate(qty4, px4, qty6, px6, "BTCUSDT", "QTUMBTC", 'xxxx', 'sell');
							order = null;
						} else {
							ord1 = {px: px4, qty: qty4, symbol: 'BTCUSDT'};
							ord2 = {px: px6, qty: qty6, symbol: 'QTUMBTC'};
							fnConsulta(qty4, px4, qty6, px6, "BTCUSDT", "QTUMBTC", 'Consulta Ordinaria INI', result2, 'sell');
						}
				  }	
			...
			if(((result > 0 ) || (result2 > 0 )) && !sw){
				...
				fsLauncher.appendFileSync('./QTUMBTC.txt', " [ " + (acum) + " :: " + (acum2) + " ]\n", (err) => {
							if (err) throw err;
								////console.log('The "data to append" was appended to file!');
							});
			}
...
function fnRemateFallo(){
	...
	//console.log('qty2: ' + ord1.qty);
		fsLauncher.appendFileSync('./QTUMBTC.txt',"ORDEN VENTA, " + ord1.symbol + ", " + ord1.qty + ", " + ord1.px + " \n", (err) => {
						if (err) throw err;
							////console.log('The "data to append" was appended to file!');
						});
	...
	binance[opG](ord1.symbol, ord1.qty, ord1.px, {type:'LIMIT'}, (error, response) => {
	  ...
	  if(error && error.body){
		console.log(error.body);
		fsLauncher.appendFileSync('./QTUMBTC.txt', ord1.symbol + '\n' + JSON.stringify(error.body) + " \n", (err) => {
				if (err) throw err;
					////console.log('The "data to append" was appended to file!');
				});  
	  } else {
	...
	fsLauncher.appendFileSync('./QTUMBTC.txt',"ORDEN VENTA, " + ord2.symbol + ", " + ord2.qty + ", " + ord2.px + " \n", (err) => {
						if (err) throw err;
							////console.log('The "data to append" was appended to file!');
						});
	binance.sell(ord2.symbol, ord2.qty, ord2.px, {type:'LIMIT'}, (error, response) => {
		...
		if(error && error.body){
			console.log(error.body);
			fsLauncher.appendFileSync('./QTUMBTC.txt', ord2.symbol + '\n' + JSON.stringify(error.body) + " \n", (err) => {
				if (err) throw err;
					////console.log('The "data to append" was appended to file!');
				});  
		} else {
...
function fnEspera(){
	...
	for(let str in objOrdenRemateFallo){
		...
		binance.orderStatus(ordA.symbol, ordA.orderId, (error, orderStatus, symbol) => {
			...
			fsLauncher.appendFileSync('./QTUMBTC.txt', symbol + " order status: " + JSON.stringify(orderStatus) + " \n", (err) => {
					if (err) throw err;
						////console.log('The "data to append" was appended to file!');
					});  
			console.log("******** consultando orden " + order.orderId + " ******* --> " + swOrd);
...
function fnRemate(qty2, px2, qty3, px3, symbol1, symbol2, msg, op){
	...
	//console.log('qty2: ' + qty2);
	fsLauncher.appendFileSync('./QTUMBTC.txt',"ORDEN VENTA, " + symbol1 + ", " + qty2 + ", " + px2 + " \n", (err) => {
						if (err) throw err;
							////console.log('The "data to append" was appended to file!');
						});
	...
	binance[op](symbol1, qty2, px2, {type:'LIMIT'}, (error, response) => {
	  ...
	  if(error && error.body){
		console.log(error.body);
		fsLauncher.appendFileSync('./QTUMBTC.txt', 'QTUMBTC\n' + JSON.stringify(error.body) + " \n", (err) => {
				if (err) throw err;
					////console.log('The "data to append" was appended to file!');
				});  
	  }
	...
	fsLauncher.appendFileSync('./QTUMBTC.txt',"ORDEN VENTA, " + symbol2 + ", " + qty3 + ", " + px3 + " \n", (err) => {
						if (err) throw err;
							////console.log('The "data to append" was appended to file!');
						});
...
function fnConsulta(qty2, px2, qty3, px3, symbol1, symbol2, msg, result, op){
	...
	if(swOrd == true){
		...
		binance.orderStatus(order.symbol, order.orderId, (error, orderStatus, symbol) => {
			console.log(symbol + " order status:", orderStatus);
			fsLauncher.appendFileSync('./QTUMBTC.txt', symbol + " DIF: " +  result + " order status: " + JSON.stringify(orderStatus) + " \n", (err) => {
					if (err) throw err;
						////console.log('The "data to append" was appended to file!');
					});  
			console.log("******** consultando orden " + order.orderId + " ******* --> " + swOrd);
			...
			if(orderStatus.status == 'FILLED'){
				...
				if(result == -10){
					fsLauncher.appendFileSync('./QTUMBTC.txt', "ORDEN TAPADA \n", (err) => {
						if (err) throw err;
							////console.log('The "data to append" was appended to file!');
						});
				} else {
