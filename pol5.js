	...
	if(swOperacion){
		...
		if(objRematablesUSDT[monedas[1]]){
			...
			/*if(monedas[0] == 'XMR'){
				...
				console.log(objCriptos['BTC_' + monedas[1]]);
			}*/
			...
			if(change > 0){
				...
				if(remate){
					...
					for(var str in obj){					
						...
						if(objCriptos['BTC_' + ref] && objCriptos[str]['lowestAsk'] && objCriptos['BTC_' + ref]['lowestAsk']){
							
							var precioTraspasando = (1 - 0.0015 / 0.9985) * remate['highestBid'] * (1 - 0.0025 / 0.9975);
							
							precioTraspasando = precioTraspasando.toFixed(8);
							var precioDirecto = objCriptos['BTC_' + ref]['lowestAsk'] * (1 + 0.0025 / 0.9975) * (((objCriptos[str]['lowestAsk'] - objCriptos[str]['highestBid']) / 2) + objCriptos[str]['highestBid']);
							...
							volOP = volRef * (1 - 0.0025 / 0.9975) / (((Number(objCriptos[str]['lowestAsk']) - Number(objCriptos[str]['highestBid'])) / 2) + Number(objCriptos[str]['highestBid']));
...
function fnDiferencia(){
	...
	if(contBooks == 3){
		...
		volRef = 0.0002 / precioReferencia;
		volRef = volRef.toFixed(8);
		...
		*/
		...
		if(order){
			...
			if(balance > 0){
				...
				console.log("DIFERENCIA (SIN ORDEN): " + (balance));
				swOperacion = false;	
			}
